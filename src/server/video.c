/*
 * GStreamer Media Player (c) 2004 Ronald Bultje
 * <rbultje@ronald.bitfreak.net> video.c: video widget. This library is
 ...
 * USA.
 *
 * Some minor modifications by Arnaldo Pereira, 02/02/2006.
 */
...
gst_player_video_class_init (GstPlayerVideoClass *klass)
{
    ...
    /* icon */
    /*redder change
      @@ filename = gnome_program_locate_file (NULL,GNOME_FILE_DOMAIN_APP_PIXMAP, "logo.png", TRUE, NULL);*/
    filename = DATA_DIR "/images/logo.jpg";
    if (filename)
        ...
        gboolean on)
{
    ...
    if (on) {
        ...
        if (GST_STATE (video->element) >= GST_STATE_PAUSED) {
            ...
            /* XXX gdk_pixbuf_unref() is deprecated, we must certainly
             * need to free logo eventually XXX */
            /*
